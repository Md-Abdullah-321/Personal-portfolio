...
function ShowContentModal({ name, subject, message, closeModal }) {
  ...
  return (
    <div ref={modalRef} id="message-dialogue" className="text-black p-4 flex flex-col justify-between h-full bg-gray-100">
      ...
      <div className="flex justify-end h-1/6">
        <button className="cursor-pointer" onClick={handleCloseModal}>Close</button>
      </div>
    </div>
  );
}
